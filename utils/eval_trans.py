...
def evaluation_time_transformer(out_dir, val_loader, net, trans, logger, writer, nb_iter, best_fid, best_iter, best_div, best_top1, best_top2, best_top3, best_matching, clip_model, eval_wrapper, dataname='t2m', draw = True, save = True, savegif=False, num_repeat=1, rand_pos=False, CFG=-1) : 
    ...
    for batch in tqdm(val_loader):
        ...
        for i in range(num_repeat):
            ...
            ######### [INFO] Eval by m_length
                pred_pose = net.vqvae(index_motion[k:k+1, :int(pred_tok_len[k].item())],type='decode')
                pred_pose_eval[k:k+1, :int(pred_len[k].item())] = pred_pose
            et_pred, em_pred = eval_wrapper.get_co_embeddings(word_embeddings, pos_one_hots, sent_len, pred_pose_eval, m_length)
            ######################################################
    ...
    return pred_pose_eval, pose, m_length, clip_text, best_fid, best_iter, best_div, best_top1, best_top2, best_top3, best_matching, multimodality, writer, logger
@torch.no_grad()        
def evaluation_time_transformer_trajectory(out_dir, val_loader, net, trans, logger, writer, nb_iter, best_fid, best_iter, best_div, best_top1, best_top2, best_top3, best_matching, clip_model, eval_wrapper, dataname='t2m', draw = True, save = True, savegif=False, num_repeat=1, rand_pos=False, CFG=-1) : 
    if num_repeat < 0:
        is_avg_all = True
        num_repeat = -num_repeat
    else:
    ...
    for batch in tqdm(val_loader):
        ...
        bs, seq = pose.shape[:2]
        speed = pose[:, :, 1:3].reshape(bs,-1, 8).cuda().to(torch.float32)
        speed = torch.cat([speed, torch.zeros(bs, 1, 8).cuda()], dim=1)
        num_joints = 21 if pose.shape[-1] == 251 else 22
        ...
        pose_wo_speed = pose.cuda().float()
        pose_wo_speed[:, :, 1:3] = 0.
        tokens = net.vqvae.teacher_net(pose_wo_speed,type='encode')
        for i in range(num_repeat):
            pred_pose_eval = torch.zeros((bs, seq, pose.shape[-1])).cuda()
            # pred_len = torch.ones(bs).long()
...
def evaluation_transformer_uplow(out_dir, val_loader, net, trans, logger, writer, nb_iter, best_fid, best_iter, best_div, best_top1, best_top2, best_top3, best_matching, clip_model, eval_wrapper, dataname, draw = True, save = True, savegif=False, num_repeat=1, rand_pos=False, CFG=-1) : 
    ...
    trans.train()
    return pred_pose_eval, pose, m_length, clip_text, best_fid, best_iter, best_div, best_top1, best_top2, best_top3, best_matching, multimodality, writer, logger
def evaluation_transformer_multi(out_dir, val_loader, net, trans, logger, writer, nb_iter, best_fid, best_iter, best_div, best_top1, best_top2, best_top3, best_matching, clip_model, eval_wrapper, dataname, draw = True, save = True, savegif=False, num_repeat=1, rand_pos=False, CFG=-1) : 
    from utils.humanml_utils_v2 import HML_UPPER_BODY_MASK, HML_LOWER_BODY_MASK,HML_LEFT_ARM_MASK, HML_RIGHT_ARM_MASK, HML_LEFT_LEG_MASK, HML_RIGHT_LEG_MASK,HML_SPINE_MASK
    ...
    matching_score_real = 0
    matching_score_pred = 0
    masks=[HML_LEFT_ARM_MASK, HML_RIGHT_ARM_MASK, HML_LEFT_LEG_MASK, HML_RIGHT_LEG_MASK, HML_SPINE_MASK]
    ...
    for batch in tqdm(val_loader):
        ...
        target=[]
        for k in range(bs):
            _target = net(pose[k:k+1, :m_length[k]], type='encode')
            if m_tokens_len[k]+1 < max_motion_length:
                _target = torch.cat([_target, 
                                    torch.ones((1, 1, 5), dtype=int, device=_target.device) * mot_end_idx, 
                                    torch.ones((1, max_motion_length-1-m_tokens_len[k].int().item(), 5), dtype=int, device=_target.device) * mot_pad_idx], axis=1)
            else:
                _target = torch.cat([_target, 
                                    torch.ones((1, 1, 5), dtype=int, device=_target.device) * mot_end_idx], axis=1)
            target.append(_target)
        target = torch.cat(target, axis=0)

        for i in range(num_repeat):
            for target_pred in range(5):
                pred_pose_eval = torch.zeros((bs, seq, pose.shape[-1])).cuda()
                # pred_len = torch.ones(bs).long()

                index_motion = trans(feat_clip_text, target, word_emb,target_pred,type="sample", m_length=pred_len, rand_pos=rand_pos, CFG=CFG)
                # [INFO] 1. this get the last index of blank_id
                # pred_length = (index_motion == blank_id).int().argmax(1).float()
                # [INFO] 2. this get the first index of blank_id
                pred_length = (index_motion >= blank_id).int()
                ...
                ######### [INFO] Eval by m_length
                    all_tokens = target[k:k+1,:int(pred_tok_len[k].item())].clone()
                    all_tokens[..., target_pred] = index_motion[k:k+1, :int(pred_tok_len[k].item())]
                    pred_pose = net(all_tokens, type='decode')
                    pred_pose_eval[k:k+1, :int(pred_len[k].item())] = pred_pose

                pred_pose_eval[..., ~masks[target_pred]] = pose[...,~masks[target_pred]]
                et_pred, em_pred = eval_wrapper.get_co_embeddings(word_embeddings, pos_one_hots, sent_len, pred_pose_eval, m_length)
                ######################################################
