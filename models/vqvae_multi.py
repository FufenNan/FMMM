...
import torch
import numpy as np  
from utils.humanml_utils import HML_UPPER_BODY_MASK, HML_LOWER_BODY_MASK, UPPER_JOINT_Y_MASK,HML_LEFT_ARM_MASK,HML_RIGHT_ARM_MASK,HML_LEFT_LEG_MASK,HML_RIGHT_LEG_MASK,OVER_LAP_LOWER_MASK,OVER_LAP_UPPER_MASK
...
class VQVAE_MULTI(nn.Module):
    def __init__(self,
        ...
        if self.sep_decoder:
            self.decoder_left_arm = Decoder(arm_dim, int(code_dim/2), down_t, stride_t, width, depth, dilation_growth_rate, activation=activation, norm=norm)
            self.decoder_right_arm = Decoder(arm_dim, int(code_dim/2), down_t, stride_t, width, depth, dilation_growth_rate, activation=activation, norm=norm)        
            self.decoder_right_leg = Decoder(leg_dim, int(code_dim/2), down_t, stride_t, width, depth, dilation_growth_rate, activation=activation, norm=norm)
            self.decoder_left_leg = Decoder(leg_dim, int(code_dim/2), down_t, stride_t, width, depth, dilation_growth_rate, activation=activation, norm=norm)        
        else:
        ...
        self.encoder_left_arm = Encoder(arm_dim, int(code_dim/2), down_t, stride_t, width, depth, dilation_growth_rate, activation=activation, norm=norm)
        self.encoder_right_arm = Encoder(arm_dim, int(code_dim/2), down_t, stride_t, width, depth, dilation_growth_rate, activation=activation, norm=norm)        
        self.encoder_right_leg = Encoder(leg_dim, int(code_dim/2), down_t, stride_t, width, depth, dilation_growth_rate, activation=activation, norm=norm)
        self.encoder_left_leg = Encoder(leg_dim, int(code_dim/2), down_t, stride_t, width, depth, dilation_growth_rate, activation=activation, norm=norm)        
        
        self.quantizer_left_arm = QuantizeEMAReset(nb_code, int(code_dim/2), args)
        self.quantizer_right_arm = QuantizeEMAReset(nb_code, int(code_dim/2), args)
        self.quantizer_right_leg = QuantizeEMAReset(nb_code, int(code_dim/2), args)
        self.quantizer_left_leg = QuantizeEMAReset(nb_code, int(code_dim/2), args)
    ...
    def merge_upper_lower(self,left_arm_emb, right_arm_emb, right_leg_emb, left_leg_emb):
        ...
        motion[..., OVER_LAP_LOWER_MASK] = (right_leg_emb[...,self.lower_map] + left_leg_emb[...,self.lower_map]) / 2
        motion[..., OVER_LAP_UPPER_MASK] = (right_arm_emb[...,self.upper_map] + left_arm_emb[...,self.upper_map]) / 2
        return motion
    ...
    def forward(self, x, *args, type='full', **kwargs):
        ...
        if type=='full':
            ...
            loss = loss_left_arm + loss_right_arm + loss_left_leg + loss_right_leg
            ...
            if self.sep_decoder:
                ...
                x_out = self.merge_upper_lower(x_d_left_arm, x_d_right_arm, x_d_right_leg, x_d_left_leg)
                x_out = self.shift_upper_up(x_out)

            else:
                x_quantized = torch.cat([left_arm_emb, right_arm_emb, right_leg_emb, left_leg_emb], dim=1)
            ...
            code_idx = torch.cat([left_arm_code_idx.unsqueeze(-1), right_arm_code_idx.unsqueeze(-1), left_leg_code_idx.unsqueeze(-1), right_leg_code_idx.unsqueeze(-1)], dim=-1)
            return code_idx
            ...
            if self.sep_decoder:
                ...
                x_d_left_leg = self.quantizer_left_leg.dequantize(x[..., 2])
                x_d_right_leg = self.quantizer_right_leg.dequantize(x[..., 3])
                x_d = torch.cat([x_d_left_arm, x_d_right_arm, x_d_right_leg, x_d_left_leg], dim=-1)
