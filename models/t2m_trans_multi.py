...
class Text2Motion_Transformer(nn.Module):
    ...
    def forward_function(self,idx,clip_feature, src_mask=None, att_txt=None, txt_mark=None, word_emb=None):
        # MLD:
        # if att_txt is None:
        #     att_txt = torch.tensor([[True]]*src_mask.shape[0]).to(src_mask.device)
        # src_mask = torch.cat([att_txt, src_mask],  dim=1)
        ...
        #[bs,50,6,512]
        feat = self.trans_base(idx, clip_feature, src_mask, word_emb)
    ...
    def sample(self, clip_feature, idx, word_emb, target, m_length=None, if_test=False, rand_pos=False, CFG=-1):
        ...
        idx_target = torch.full(shape, mask_id, dtype = torch.long, device = clip_feature.device)
        ...
        for step in range(max_steps):
            ...
            idx_target.scatter_(-1, sorted_score_indices, mask_id)
            ...
            if CFG!=-1:
                ...
                logits = self.forward(_ids, idx, _clip_feature, _src_token_mask, att_txt)[:,1:]
                ...
                input_idx=idx.clone()
                input_idx[...,target]=idx_target
                logits = self.forward(input_idx, clip_feature, src_token_mask, word_emb=word_emb)[:,1:]
            ...
            filtered_logits = filtered_logits[:,:,target,:].squeeze(2)
            pred_ids = gumbel_sample(filtered_logits, temperature = temperature, dim = -1)
            is_mask = idx_target == mask_id
...
class CrossCondTransBase(nn.Module):
    ...
    def __init__(self, 
        ...
        self.learn_tok_emb = nn.Embedding(3, int(self.vqvae.vqvae.code_dim/5))# [INFO] 3 = [end_id, blank_id, mask_id]
    ...
    def forward(self,idx, clip_feature, src_mask, word_emb):
        ...
        if len(idx[...,0]) == 0:
            ...
            token_embeddings = torch.empty((*idx[...,0].shape, code_dim), device=idx[0].device)
            quantizers = [
                self.vqvae.vqvae.quantizer_left_arm,
                self.vqvae.vqvae.quantizer_right_arm,
                self.vqvae.vqvae.quantizer_left_leg,
                self.vqvae.vqvae.quantizer_right_leg,
                self.vqvae.vqvae.quantizer_spine
            ]
            for i in range(5):
                token_embeddings[..., int(i*code_dim/5):int((i+1)*code_dim/5)][~learn_idx[i]] = quantizers[i].dequantize(idx[...,i][~learn_idx[i]]).requires_grad_(False)
                token_embeddings[..., int(i*code_dim/5):int((i+1)*code_dim/5)][learn_idx[i]] = self.learn_tok_emb(idx[...,i][learn_idx[i]] - self.vqvae.vqvae.num_code)
            token_embeddings = self.to_emb(token_embeddings)  # [bs,50,5*256]
...
class CrossCondTransHead(nn.Module):
    ...
    def __init__(self, 
        ...
        self.blocks = nn.Sequential(*[Block(embed_dim, block_size, n_head, drop_out_rate, fc_rate) for _ in range(num_layers)])
        self.ln_f = nn.LayerNorm(embed_dim//5)
        self.head = nn.Linear(embed_dim//5, num_vq, bias=False)
        self.block_size = block_size
    ...
    def forward(self, x, src_mask):
        ...
        x = x.reshape(batch_size,seq_len,5,embed_dim // 5)
        x = self.ln_f(x)
        logits = self.head(x)
        return logits
