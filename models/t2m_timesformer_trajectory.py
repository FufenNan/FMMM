...
class Time_Attention(nn.Module):
    ...
    def forward(self, x, einops_from, einops_to, mask = None, rot_emb = None, **einops_dims):
        ...
        if exists(rot_emb):
            #TODO check apply_rot_emb
            for i in range(len(rot_emb)):
                if rot_emb[i].shape[0]!=bs and rot_emb[i].shape[0]!=1:
                    rot_emb[i] = rot_emb[i].repeat(bs//rot_emb[i].shape[0], 1, 1)
...
class TimeSformer(nn.Module):
    ...
    def forward(self, video, mask = None, speed=None):
...
class Text2Motion_Transformer(nn.Module):
    ...
    def forward_function(self, idxs, clip_feature, src_mask=None, att_txt=None, word_emb=None,speed=None):
        ...
        logits = self.trans_head(feat, src_mask, speed)
    ...
    def sample(self, tokens, clip_feature, word_emb=None, m_length=None, if_test=False, rand_pos=True, CFG=-1, token_cond=None, max_steps=10, speed = None):
        ...
        for step in range(max_steps):
            ...
            logits = self.forward(ids.permute(0,2,1), clip_feature, src_token_mask[...,0], word_emb=word_emb, speed = speed)[:, 1:]
...
class CrossCondTransBase(nn.Module):
    ...
    def forward(self,idx, clip_feature, src_mask, word_emb,speed=None):
        #TODO
        if len(idx) == 0:
...
class CrossCondTransHead(nn.Module):
    ...
    def forward(self, x, src_mask,speed=None):
        if len(x.shape) == 4:
            x = x.squeeze(2)
        for block in self.blocks:
            x = block(x, src_mask,speed)
